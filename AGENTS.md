...
| 专题步骤 | `docs/主题-*.md` | `CONTENT-FORMAT.md` |
...
npm run dev:color  # 实验配色：改 theme-experiment.css，不动 theme.css
...
> **施工图** = [P1-MIX-MATCH-LIST.md](docs/P1-MIX-MATCH-LIST.md) · **活进度** = [STATUS.md](docs/STATUS.md) · **线框** = [SITE-MANUAL.md](docs/SITE-MANUAL.md)
...
| **P1-MIX-MATCH-LIST.md** | **P1 执行方案** — 功能块 × 偷哪个 demo × §6 A/B/C 顺序 × §8 验收 |
| **STATUS.md** | **现在第几步** — 主线 ①–⑥、最近决策 |
| **SITE-MANUAL.md** | **站现状线框** — 改 `src/` 布局后必须同步 |
| **sessions/YYYY-MM-DD** | 当天叙事收工；**施工清单仍以 P1-MIX-MATCH §8 为准** |

P1-MIX-MATCH 由 §6 拍板 + 试吃笔记**编译而成**；执行时继续参照二者，不是每次从零写方案。

### 三阶段顺序（P1-MIX-MATCH §6.1）

| 阶段 | 做什么 | 验收 |
|------|--------|------|
| **A · 骨架** | 六页分区、栅格、占位 | 对照 SITE-MANUAL 能一眼看出块 |
| **B · 视觉** | 按 demo 填皮（色、字、气质） | 对照 `design-demos/` + 试吃笔记 |
| **C · 功能动效** | 搜索、筛选、hover 等 | P1-MIX-MATCH §8 逐项 |

**原则**：A 未收工不在 C 上堆新功能（§6.5 已复盘「零碎先行」偏离）。**现状**：A ✅ 收工 · **B 进行中**（见 P1-MIX-MATCH §8.9）。

### 动手改一块时的步骤

1. 读 **STATUS** — 确认在 P1 主线、当前 A/B/C
2. 查 **P1-MIX-MATCH §8.x** — 块标签（如 A1-5）、对照 demo
...
6. 收工：**SITE-MANUAL**（动布局）→ **STATUS** + P1-MIX-MATCH §8.9（B 进度）→ 必要时 sessions
...
| [docs/AI-COLLABORATION-CONSTRAINTS.md](docs/AI-COLLABORATION-CONSTRAINTS.md) | **跨项目可复用规则约束表**入口 → Skill `COLLABORATION-CONSTRAINTS-TABLE.md` |
...
| [docs/P1-MIX-MATCH-LIST.md](docs/P1-MIX-MATCH-LIST.md) | P1 视觉混搭施工图（§6 拍板后） |
| [docs/MENTAL-MODELS-IA.md](docs/MENTAL-MODELS-IA.md) | `/galaxy` 圈层 IA · 节点归属 · P3 助手同源数据 |
| [docs/CONTENT-FORMAT.md](docs/CONTENT-FORMAT.md) | **站内内容排版**（日期·标题·摘要·延展·字幕 §D） |
| [docs/MINDMAP-RULES.md](docs/MINDMAP-RULES.md) | **详情思维导图**（抽取·层级·布局·连线·自检；Archie+XMind） |
| [docs/P2-CONTENT-INBOX.md](docs/P2-CONTENT-INBOX.md) | **P2 捡料**施工图（字幕上站 · 外链 · 批次） |
| [docs/P3-ASSISTANT-SPIKE.md](docs/P3-ASSISTANT-SPIKE.md) | **P3 尖兵**施工图（DeepSeek 通聊试点 · A/B/C） |
| [docs/P3-AUTHOR-NOTE-SPIKE.md](docs/P3-AUTHOR-NOTE-SPIKE.md) | **亲笔旁注站内编辑**（云真源 · 防 Git 覆盖） |
...
| 北京时间 | 变更 |
|----------|------|
| 2026-07-17 15:07:26 | 文档索引补 **MINDMAP-RULES** |
| 2026-07-17 10:34:45 | 删除 stub · 唯一权威 [`CONTENT-FORMAT.md`](docs/CONTENT-FORMAT.md) |
| 2026-07-17 10:24:32 | **合并** 字幕+文案硬规则 → CONTENT-FORMAT |
| 2026-07-17 09:26:37 | 文档索引补 **P2-CONTENT-INBOX** |
| 2026-07-22 11:34:17 | 文档索引补 **P3-AUTHOR-NOTE-SPIKE** |
| 2026-07-14 15:06:57 | **更名** MIX-MATCH → **P1-MIX-MATCH-LIST.md** |
| 2026-07-14 14:20:14 | 文档索引补 **P3-ASSISTANT-SPIKE** |
| 2026-07-14 09:12:32 | 文档索引补 **TENCENT-CLOUD-DEPLOY** · `/galaxy` 螺旋布局（部署指南后已删） |
